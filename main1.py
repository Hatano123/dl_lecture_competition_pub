...
def collate_fn(batch):
    ...
    answers_padded = pad_sequence(answers, batch_first=True, padding_value=-1)
...
class VQADataset(torch.utils.data.Dataset):
    def __init__(self, df_path, image_dir, transform=None, answer=True):
        ...
        sorted_words = sorted(self.word_counter.items(), key=lambda x: x[1], reverse=True)
        for idx, (word, _) in enumerate(sorted_words):
            self.question2idx[word] = idx + 1  # 0をPAD用に予約
    ...
    def __getitem__(self, idx):
        ...
        question_tokens = nltk.word_tokenize(process_text(self.df["question"][idx]))
        question_indices = [self.question2idx.get(token, 0) for token in question_tokens]  # 未知語は0(PAD)に
...
class VQAModel(nn.Module):
    def __init__(self, vocab_size: int, n_answer: int, embed_dim: int = 300):
        super().__init__()
        self.resnet = ResNet18()
        self.embedding = nn.Embedding(vocab_size, embed_dim)
        self.text_encoder = nn.LSTM(embed_dim, 512, batch_first=True)
    ...
    def forward(self, image, question):
        ...
        # 埋め込み層を通して分散表現を取得
        question_embed = self.embedding(question.long())
        
        # LSTMを通して質問文の特徴量を取得
        _, (hidden, _) = self.text_encoder(question_embed)
